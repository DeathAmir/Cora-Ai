...
function json_input(): array {
    ...
    if ($raw === false || strlen($raw) > 100000) json_response(['error' => 'درخواست بیش از حد بزرگ است.'], 413);
...
function require_csrf(array $data = []): void {
    ...
    if (!is_string($token) || !hash_equals($_SESSION['csrf'] ?? '', $token)) json_response(['error' => 'نشست معتبر نیست. صفحه را تازه‌سازی کنید.'], 419);
}
...
function clean_text(string $value, int $max): string { $value = trim(preg_replace('/\x00/u', '', $value)); return mb_substr($value, 0, $max); }
...
function rate_limit(string $action, int $limit, int $windowSeconds): void {
    ...
    try {
        $stmt = $pdo->prepare('SELECT id, hits, window_start FROM rate_limits WHERE bucket_key=? AND action_name=? FOR UPDATE');
        $stmt->execute([$key, $action]);
        $row = $stmt->fetch();
        $now = time();
        if (!$row) {
            $pdo->prepare('INSERT INTO rate_limits(bucket_key, action_name, hits, window_start) VALUES(?,?,1,NOW())')->execute([$key, $action]);
        } else {
            ...
            if ($now - $start >= $windowSeconds) {
                ...
                $pdo->rollBack();
                json_response(['error' => 'درخواست‌های زیادی ارسال شده. کمی بعد دوباره تلاش کنید.'], 429);
            } else {
...
function conversation_owner(int $conversationId, int $userId): array {
    $stmt = db()->prepare('SELECT * FROM conversations WHERE id=? AND user_id=? AND deleted_at IS NULL');
    $stmt->execute([$conversationId, $userId]);
    $row = $stmt->fetch();
    if (!$row) json_response(['error' => 'گفتگو پیدا نشد.'], 404);
    return $row;
}
